...
int probe_start(struct pt_regs *ctx)
{
    ...
    if (err < 0) {
        bpf_printk("start: update map failed %d\n", err);
        return err;
    }
...
int probe_end(struct pt_regs *ctx)
{
    ...
    if (pval) {
        ...
        if (err < 0) {
            bpf_printk("end: delete map failed %d  %u  %u\n", err, pid, *pval);
            return err;
        }
    } else {
        bpf_printk("end bad thing happens pid:%d  *pval:%u\n", pid, *pval);
    }
    return 0;
}
...
int probe_kmem_cache_alloc_file(struct pt_regs *ctx)
{
    ...
    u32 pid = bpf_get_current_pid_tgid();
    err = allocation(alloc_size, 0, pid);
    if (err < 0) 
        return -1;

...
void MLTA::findCalleesWithType(CallInst *CI, FuncSet &S) {
	...
	for (Function *F : MLTACtx->AddressTakenFuncs) {
		// VarArg
			...
			for (User::op_iterator OI = I->op_begin(), 
				...
				if (Function *F = dyn_cast<Function>(*OI)) {
					...
					if (!CF)
						continue;
					if (CF->isDeclaration())
						CF = MLTACtx->GlobalFuncMap[CF->getGUID()];
					if (!CF)
						continue;
...
bool MLTA::findCalleesWithMLTA(CallInst *CI, 
	...
	FS = MLTACtx->sigFuncsMap[callHash(CI)];
	...
	if (LayerNo > 1) {
		MLTACtx->NumSecondLayerTypeCalls++;
		MLTACtx->NumSecondLayerTargets += FS.size();
	}
	else {
		MLTACtx->NumFirstLayerTargets += MLTACtx->sigFuncsMap[callHash(CI)].size();
		MLTACtx->NumFirstLayerTypeCalls += 1;
	}

#if 0
	FuncSet FSBase = MLTACtx->sigFuncsMap[callHash(CI)];

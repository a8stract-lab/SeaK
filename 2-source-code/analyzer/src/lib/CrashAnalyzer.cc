...
void CrashAnalyzer::runOnFunction(Function *F) {
  ...
  for (inst_iterator i = inst_begin(F), e = inst_end(F); i != e; i++) {
    ...
    if (line == Loc->getLine() && sourceF == source) {
      // FIXME: sometimes the source starts from somewhere weired,
      // so we go through all the possible functions.
      // one measurement is to link the bitcodes and analyze the linked one.
      // analyzed = true;

      KA_LOGS(0, "Analyzing crash loc: " << *I);
          ...
          if (Fname.find("printk") != string::npos || CI->isInlineAsm()) {
            ...
            while (BB->getSinglePredecessor()) {
              ...
              for (Instruction &BBInst : *BB) {
                if (isa<StoreInst>(&BBInst)) {
                  ...
                  if (isa<ConstantInt>(SI->getOperand(0)) &&
                    ...
                    StringRef GVName =
                        cast<GlobalVariable>(SI->getOperand(1))->getName();
                    KA_LOGS(0, "Found Global Value: " << GVName);
                    if (GVName.find("__warned") != string::npos && F &&
                        F->getName().find("__warn_printk") != string::npos) {
              ...
              if (isa<BranchInst>(newBB->getTerminator())) {
                ...
                if (BI->isConditional()) {
                  ...
                  for (auto &newBBInst : *newBB) {
                    if (isa<LoadInst>(&newBBInst)) {
                      LoadInst *LI = cast<LoadInst>(&newBBInst);
                      if (isa<GlobalVariable>(LI->getOperand(0))) {
                        StringRef GVName =
                            cast<GlobalVariable>(LI->getOperand(0))->getName();
                        KA_LOGS(0, "Found Global Value: " << GVName);
                        if (GVName.find("debug") !=string::npos) {
                          KA_LOGS(0, "Warn ONCE checking here....");
                          warnOnce = true;
                        }
                      }
            ...
            while (!BBVec.empty() && !SecondCondV) {
              ...
              if (!BB->hasNPredecessorsOrMore(1)) {
                ...
                if (Ctx->CallGraph.find(fName.str()) != Ctx->CallGraph.end()) {
                  auto callerName = Ctx->CallGraph[fName.str()];
                  for (auto M : Ctx->Callers) {
                    if (M.first->getName().endswith(fName)) {
                      auto CISet = M.second;
                      for (CallInst *caller : CISet) {
                        if (caller->getFunction()->getName() == callerName) {
                          BB = caller->getParent();
                          KA_LOGS(0,
                                  "Backwarding to its parent " << callerName);
                          break;
                        }
                      }
                    }
              ...
              for (BasicBlock *Pred : predecessors(BB)) {
                ...
                if (isa<BranchInst>(Pred->getTerminator())) {
                  ...
                  if (BI->isConditional() && condV != BI->getCondition()) {
...
void CrashAnalyzer::dump() { dump("/tmp/ca_result"); }
...
void CrashAnalyzer::dump(StringRef outFile) {
  ...
  if (!cond && Ctx->InstNum > 1) {
    KA_WARNS(
        0,
        "Please take a look at this case, find more than one load and store");
    sleep(2);
  }
...
bool CrashAnalyzer::doInitialization(Module *M) { return false; }
...
bool CrashAnalyzer::doFinalization(Module *M) { return false; }

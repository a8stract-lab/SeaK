...
cl::opt<std::string> CrashReport("crash-report", cl::desc("crash report"),
                                 cl::NotHidden, cl::init(""));
...
//     "check-leakers", cl::desc("Analyze leakers"), cl::NotHidden,
//     cl::init(false));
...
//     "ignore-reachable", cl::desc("Ignore whether the function is reachable
//     from syscall"), cl::NotHidden, cl::init(false));

cl::opt<std::string> CallGraph("call-graph",
                               cl::desc("call graph from the report"),
                               cl::NotHidden, cl::init(""));
...
int main(int argc, char **argv) {
  ...
  if (StructAlloc != "") {
    AllocationFinderPass AFPass(&GlobalCtx, StringRef(StructAlloc));
    AFPass.run(GlobalCtx.Modules);
    AFPass.dump();

    FreeFinderPass FFPass(&GlobalCtx, StringRef(StructAlloc));
  ...
  KA_LOGS(0, "[+] Identify crash location as " << CrashLoc << " explicit checking? "
                                           << explicity);

  // CrashAnalyzer CA(&GlobalCtx, isExplicit, "refcount_inc_checked",
  // "lib/refcount.c", 156);
  assert(CrashLoc != "");
  {
  ...
  // AllocationFinderPass AFPass(&GlobalCtx);
  // AFPass.run(GlobalCtx.Modules);

  // SimplePtrToAnalysisPass SPTPass(&GlobalCtx);
  // SPTPass.run(GlobalCtx.Modules);

  // outs() << "We got the crash location: " << CrashLoc << " explicit checking?
  // "
  //        << explicity << "\n";

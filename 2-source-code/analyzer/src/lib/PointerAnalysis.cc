...
bool PointerAnalysisPass::doInitialization(Module *M) { return false; }

bool PointerAnalysisPass::doFinalization(Module *M) { return false; }
...
void PointerAnalysisPass::detectAliasPointers(Function *F, AAResults &AAR,
  ...
  for (auto Addr1 : addr1Set) {
    for (auto Addr2 : addr1Set) {
      ...
      if (AResult == AliasResult::MustAlias || AResult == AliasResult::PartialAlias) {
        notAlias = false;
      } else if (AResult == AliasResult::MayAlias) {
      }

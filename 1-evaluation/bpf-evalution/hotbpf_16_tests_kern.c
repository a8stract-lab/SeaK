...
int probe_kmalloc(struct pt_regs *ctx)
{
	...
	// if (is_target_alloc_site(ip)) {
		...
		bpf_jmp_next(ctx, 0xffffffff81b81b74, alloc_addr);
	// }
...
int probe_kmalloc1(struct pt_regs *ctx)
{
	...
	// if (is_target_alloc_site(ip)) {
		...
		bpf_jmp_next(ctx, 0xffffffff81d06f58, alloc_addr);
	// }
...
int probe_kmalloc2(struct pt_regs *ctx)
{
	...
	bpf_jmp_next(ctx, 0xffffffff819a488f, alloc_addr);
...
int probe_kmalloc3(struct pt_regs *ctx)
{
	...
	// 	err = bpf_override_return(ctx, (unsigned long)alloc_addr);
	// }
	bpf_jmp_next(ctx, 0xffffffff81a6d666, alloc_addr);
...
int probe_kmalloc4(struct pt_regs *ctx)
{
	...
	// err = bpf_override_return(ctx, (unsigned long)alloc_addr);
	bpf_jmp_next(ctx, 0xffffffff81d2cf42, alloc_addr);
	// }
	return 0;
}
...
int probe_kmalloc5(struct pt_regs *ctx)
{
	...
	// 	err = bpf_override_return(ctx, (unsigned long)alloc_addr);
	// }
	bpf_jmp_next(ctx, 0xffffffff81b414c3, alloc_addr);
...
int probe_kmalloc6(struct pt_regs *ctx)
{
	...
	// if (is_target_alloc_site(ip)) {
		...
		// err = bpf_override_return(ctx, (unsigned long)alloc_addr);
		bpf_jmp_next(ctx, 0xffffffff81ad9331, alloc_addr);
	// }
	return 0;
}
...
int probe_kmalloc72(struct pt_regs *ctx)
{
	...
	bpf_jmp_next(ctx, 0xffffffff81b7beef, alloc_addr);
...
int probe_kmalloc8(struct pt_regs *ctx)
{
	...
	// if (is_target_alloc_site(ip)) {
		...
		// err = bpf_override_return(ctx, (unsigned long)alloc_addr);
	// }
	bpf_jmp_next(ctx, 0xffffffff81da2b82, alloc_addr);
...
int probe_kmalloc9(struct pt_regs *ctx)
{
	...
	// 	err = bpf_override_return(ctx, (unsigned long)alloc_addr);
	// }
	bpf_jmp_next(ctx, 0xffffffff814e3bfe, alloc_addr);

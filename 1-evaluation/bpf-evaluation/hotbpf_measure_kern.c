...
int measure_init(struct pt_regs* ctx)
{
	// if (cache == 0)
	// 	cache = bpf_create_slab_cache(8192, GFP_KERNEL, 0);

	// alloc_addr1 = bpf_cache_alloc(cache, GFP_KERNEL);

	u64 ip = 0;
	u64 *pv;
	u64 alloc_size = 0;
	u64 alloc_addr = 0;
	u32 gfp_flags = GFP_KERNEL;
	u64 key = 0;
	u32 uid = 0;
	u32 zone = 0;
	u64 cache_addr = 0;
...
// ffffffff810035e0 <hackme_read>:
// ...
// ffffffff8100369a:       0f 31                   rdtsc
// ffffffff8100369c:       48 89 d1                mov    %rdx,%rcx
// ffffffff8100369f:       48 89 c6                mov    %rax,%rsi
// ffffffff810036a2:       0f 31                   rdtsc
// python3 -c 'print(hex(0xffffffff810036a2 - 0xffffffff810035e0))'
SEC("kprobe/hackme_read+0xc2")
int measure(struct pt_regs* ctx)
{
	// if (alloc_addr != 0)
	// 	bpf_cache_free(alloc_addr);
	// bpf_set_pt_present((alloc_addr), 1);
	u64 ip = 0;
	u64 pid = 0;
	u64 ip_size_priv = 0;
	u64 alloc_size = 0;

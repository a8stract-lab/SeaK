...
// Allocation APIs
static std::set<llvm::StringRef> allocAPIVec = {
    ...
    "sock_kmalloc", "kasprintf",
    };

static std::set<llvm::StringRef> freeAPIVec = { "kfree", "kvfree", "kvfree_call_rcu", "kfree_call_rcu" "kzfree", "kmem_cache_free" };

static StrSet skipFunc = {"debug_print_object",
                          "debug_object_init",
                          "debug_assert_init", /*ODEBUG*/
                          "__dump_stack",
                          "dump_stack",
...
static StrSet BlockSt = {
    ...
    // // %work = getelementptr inbounds %struct.rcu_work, %struct.rcu_work*
    // %rwork,
...
public:
  ...
  // element offset information
  DenseMap<llvm::StringRef, std::set<unsigned int>> ElementOffset;

  // calls of allocation we are looking for
  CallInstSet Allocations;

  // calls of free we are looking for
  CallInstSet Frees;

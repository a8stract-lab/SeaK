...
def get_calls():
    ...
    if "WARNING" in report or "GPF" in report or "kernel BUG at" in report \
        ...
        if found:
            # print(found.group(1)+found.group(2))
            return found.group(1)+found.group(2)
    if "kasan" in report or "UBSAN" in report or "KMSAN" in report:
    ...
    return ""
"""
non_infomative_func = ["__list_add", "list_add_tail", "kobj_kset_join", "kobject_add_internal", "kobject_add_varg", "kobject_add", "device_add", "__list_del_entry_valid", "__list_del_entry", "list_del_rcu"]
def non_informative(call):
    for garbage_func in non_infomative_func:
        if call.find(garbage_func) != -1:
            return True
    return False
"""
